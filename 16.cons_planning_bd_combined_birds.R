...
# prep feature data
features <- here::here("outputs/BinaryRasters_Built-upAreasRemoved_combined/birds/") %>%
...
if(!file.exists("data/rij_birds_combined.rds")){
  rij <- rij_matrix(pu, features)
  rij %>% saveRDS("data/rij_birds_combined.rds", compress = FALSE) 
  
} else {
  
  rij <- readRDS("data/rij_birds_combined.rds") 
...
# Best objective 5.741782255096e+05, best bound 5.734635971253e+05, gap 0.1245%
#################################
...
# save results
r2 %>% writeRaster("outputs/spatial_prioritization_birds_combined.tif", overwrite = TRUE)
...
ir_raster %>% writeRaster("outputs/irreplacecable_areas_birds_combined.tif", overwrite = TRUE)
