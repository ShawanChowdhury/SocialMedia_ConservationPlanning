...
# Reading data file
sp_data <- read_csv("data/combinedRecords_thinned_GBIF.csv")
sp_data <- sp_data %>% 
...
for(i in species) try({
  ...
  dc <- sp_data %>% filter(species==speciesname)
  d.c <- dc[,2:3]
...
# List of species
files <- list.files(path = "outputs/sdm_GBIF/", pattern = "\\.csv$", recursive = TRUE, full.names = TRUE)
