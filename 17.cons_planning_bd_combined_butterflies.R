...
# prep feature data
features <- here::here("outputs/BinaryRasters_Built-upAreasRemoved_combined/butterflies/") %>%
...
if(!file.exists("data/rij_butterflies_combined.rds")){
  rij <- rij_matrix(pu, features)
  rij %>% saveRDS("data/rij_butterflies_combined.rds", compress = FALSE) 
  
} else {
  
  rij <- readRDS("data/rij_butterflies_combined.rds") 
...
# Best objective 4.986029432907e+05, best bound 4.979677897911e+05, gap 0.1274%
#################################
...
# save results
r2 %>% writeRaster("outputs/spatial_prioritization_butterflies_combined.tif", overwrite = TRUE)
...
ir_raster %>% writeRaster("outputs/irreplacecable_areas_butterflies_combined.tif", overwrite = TRUE)
