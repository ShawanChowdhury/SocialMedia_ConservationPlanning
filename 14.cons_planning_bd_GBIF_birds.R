...
# prep feature data
features <- here::here("outputs/BinaryRasters_Built-upAreasRemoved_GBIF/birds/") %>%
...
if(!file.exists("data/rij_birds_GBIF.rds")){
  rij <- rij_matrix(pu, features)
  rij %>% saveRDS("data/rij_birds_GBIF.rds", compress = FALSE) 
  
} else {
  
  rij <- readRDS("data/rij_birds_GBIF.rds") 
...
full_data <- read_csv("outputs/target_birds_GBIF.csv")
...
# Best objective 5.300854015670e+05, best bound 5.296630336312e+05, gap 0.0797%
#################################
...
# save results
r2 %>% writeRaster("outputs/spatial_prioritization_birds_GBIF.tif", overwrite = TRUE)
...
ir_raster %>% writeRaster("outputs/irreplacecable_areas_birds_GBIF.tif", overwrite = TRUE)
