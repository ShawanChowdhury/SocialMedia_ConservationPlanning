...
# prep feature data
features <- here::here("outputs/BinaryRasters_Built-upAreasRemoved_GBIF/butterflies/") %>%
...
if(!file.exists("data/rij_butt_combined.rds")){
  rij <- rij_matrix(pu, features)
  rij %>% saveRDS("data/rij_butterflies_GBIF.rds", compress = FALSE) 
  
} else {
  
  rij <- readRDS("data/rij_butterflies_GBIF.rds") 
...
full_data <- read_csv("outputs/target_butterflies_GBIF.csv")
...
# Best objective 3.874883775463e+05, best bound 3.873307870774e+05, gap 0.0407%
#################################
...
# save results
r2 %>% writeRaster("outputs/spatial_prioritization_butterflies_GBIF.tif", overwrite = TRUE)
...
ir_raster %>% writeRaster("outputs/irreplacecable_areas_butterflies_GBIF.tif", overwrite = TRUE)
