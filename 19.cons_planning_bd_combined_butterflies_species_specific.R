...
# prep feature data
features <- here::here("outputs/BinaryRasters_Built-upAreasRemoved_combined_species_specific/butterflies/") %>%
...
if(!file.exists("data/rij_butterflies_combined_species_specific.rds")){
  rij <- rij_matrix(pu, features)
  rij %>% saveRDS("data/rij_butterflies_combined_species_specific.rds", compress = FALSE) 
  
} else {
  
  rij <- readRDS("data/rij_butterflies_combined_species_specific.rds") 
...
full_data <- read_csv("outputs/target_butterflies_combined.csv")

# FIltering species data for which we only obtained model using GBIF data
gbif <- read_csv("outputs/target_butterflies_GBIF.csv")
gbif <- gbif[,4]
...
# Resample PA layer
raster_file <- raster("outputs/BinaryRasters_Built-upAreasRemoved_combined/butterflies/Anthene_emolus.tif")
...
# Best objective 3.465221399155e+05, best bound 3.459843489757e+05, gap 0.1552%
#################################
...
# save results
r2 %>% writeRaster("outputs/spatial_prioritization_butterflies_combined_species_specific.tif", overwrite = TRUE)
...
ir_raster %>% writeRaster("outputs/irreplacecable_areas_butterflies_combined_species_specific.tif", overwrite = TRUE)
